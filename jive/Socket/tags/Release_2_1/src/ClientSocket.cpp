 ...
 */
//------------------------------------------------------------
const ClientSocket& ClientSocket::operator << (const Tango::DevVarCharArray& charArray) const
{
    	if (! SocketAccess::sendChar(charArray.get_buffer(),charArray.length()))
	{
		throw SocketException ("Write Socket Failed.");
	}
 ...
 */
//------------------------------------------------------------
void ClientSocket::readCharArray(Tango::DevVarCharArray& charArray,long nbCharToRead, short timeout)
{
        charArray.length(nbCharToRead);
	char c;
        for(int i = 0; i < nbCharToRead ; i++){
...
//------------------------------------------------------------
/**
 *	ClientSocket read a binary buffer
 */
//------------------------------------------------------------
int ClientSocket::readCharArrayBuffer(char *charArray,long nbCharToRead, short timeout)
{
	...
	GetTime(t0);
#endif
	int	nb_bytes_read  = SocketAccess::recv( charArray, nbCharToRead, timeout);

// cout << "2> nb_bytes_read " << nb_bytes_read << endl;
//	cout << "2> socket data: ";
//  for (int i=0; i < 16; i++) printf("0x%02x, ", charArray[i]);
//	cout << endl;
	...
	*/
	if (nb_bytes_read  > 0)
		return nb_bytes_read ;	//	Ok on char read
	else
		throw SocketException ("Read Socket Failed");
